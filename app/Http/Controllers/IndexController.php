...

{
...

    {
        ...
        if ($request->query('data')) {
            ...
            //sent different mail based on number of donation user made------------start
            $totalDonates = Donate::where(['userId' => Session::get('user')->id])->count();
            if ($totalDonates < 2) {
                //first time 
            ...
            } elseif ($totalDonates == 2) {
                //2nd time 

                Mail::send('emails.subscription-two', $data, function ($m) use ($recipient_email, $recipient_name, $subject) {
                    $m->subject($subject);
                    $m->to($recipient_email, $recipient_name);
                });
            } else {
                ...
                Mail::send('emails.subscription-many', $data, function ($m) use ($recipient_email, $recipient_name, $subject) {
                    $m->subject($subject);
                    $m->to($recipient_email, $recipient_name);
                });
            }
            //sent different mail based on number of donation user made------------end
